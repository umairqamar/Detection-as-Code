...
def main():
    sigmaDir = sys.argv[ 1 ]
    sigmaTarget = sys.argv[ 2 ]
    sigmaBackend = sys.argv[ 3 ]
    sigmaConfig = sys.argv[ 4 ]
    rootOutputDir = sys.argv[ 5 ]
    dirsToProcess = sys.argv[ 6 : ]
        ...
        for d in dirsToProcess:
            ...
            for r, _, files in os.walk( '%srules/%s/' % ( sigmaDir, d, ) ):
                for f in files:
                    ...
                    pool.apply_async( processRule( sigmaDir, sigmaTarget,sigmaBackend, sigmaConfig, thisFile, outFile ) )
        pool.close()
        pool.join()

def processRule( sigmaDir, sigmaTarget,sigmaBackend, sigmaConfig, thisFile, outFile ):
    ...
    # Artifact rules.
    os.system( "python3 %stools/sigmac --target %s --backend-option %s --config %stools/config/%s.yml %s > %s" % ( sigmaDir, sigmaTarget, sigmaBackend, sigmaDir, sigmaConfig, thisFile, outFile ) )
    ...
    # except:
    #     pass
    # os.system( "python3 %stools/sigmac -t %s --backend-option es-qs  -c %stools/config/%s.yml %s > %s" % ( sigmaDir, sigmaTarget, sigmaDir, sigmaConfig, thisFile, outFile ) )
    # if os.path.getsize( outFile ) == 0:
    #     print( "rule %s was empty, deleting" % ( outFile, ) )
    #     os.system( 'rm %s' % ( outFile, ) )l
